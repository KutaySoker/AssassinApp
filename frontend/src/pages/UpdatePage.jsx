...
const UpdatePage = () => {
  ...
  const handleUpdate = async (id = null) => {
    ...
    eventSource.onmessage = (event) => {
      ...
      if (eventData.text) setLogText(eventData.text);

      // Burayı iyice kontrol et, done gelince loader kapansın
      if (eventData.done === true) {
        console.log("🚀 BİTTİ SİNYALİ ALINDI!");
        setTimeout(() => {
          eventSource.close();
          setUpdating(false);
          fetchUpdates(); // Listeyi tazele
        }, 2000);
      }
    };
  ...
  if (loading || updating) {
    return (
      <div className="flex flex-col items-center justify-center h-full space-y-8 animate-in fade-in duration-1000">
        ...
        <div className="text-zinc-500 font-mono text-sm tracking-widest max-w-lg text-center h-10 overflow-hidden px-4">
          {logText}
        </div>
      </div>
    );
    ...
    <div className="p-10 w-full max-w-6xl mx-auto animate-in fade-in zoom-in duration-500">
      ...
      <div className="flex justify-between items-center mb-10 border-b border-zinc-800 pb-6">
        <div>
          ...
          <p className="text-zinc-500 font-mono text-sm">Sisteminizdeki güncelliği otomatik denetleyin ve yönetin.</p>
        </div>
